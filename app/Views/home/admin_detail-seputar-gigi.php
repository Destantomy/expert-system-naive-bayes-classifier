...
<style>
    ...
    @media(max-width : 900px) {
        ...
        .tambah-artikel {
            ...
            position: absolute;
            width: 24.5rem;
            margin: 1.5rem 0 0 3.5vh;
            padding: 1.5rem;
            left: 0%;
            transition: all 0.7s ease;
        }

        .tambah-artikel.active {
            left: -300%;
        }
    }
</style>
...
<div class="tambah-artikel">
    <form action="/Admin_seputarGigi/updateDataArtikel/<?= $data_artikel['id']; ?>" method="POST">
        <?= csrf_field(); ?>
        <input type="text" name="id" value="<?= $data_artikel['id']; ?>" hidden>
        <div class="title">
        ...
        <div class="headline">
            <div class="mb-3">
                ...
                <input name="headline" type="text" autofocus value="<?= $data_artikel['headline']; ?>" class="form-control <?= ($validation->hasError('headline')) ? 'is-invalid' : ''; ?>" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Write here the article's title">
        ...
        <div class="konten">
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Konten Artikel</label>
                <textarea name="konten" class="form-control <?= ($validation->hasError('konten')) ? 'is-invalid' : ''; ?>" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Write here the article's content" cols="30" rows="10"><?php echo $data_artikel['konten']; ?></textarea>
                <div id="validationServer05Feedback" class="invalid-feedback">
                    <?= $validation->getError('konten'); ?>
                </div>
            </div>
        </div>
        <button type="submit" class="btn btn-warning" onclick="return confirm('Update data artikel?');">Update</button>
        <a href="/artikel/<?= $data_artikel['id']; ?>" class="btn btn-danger" onclick="return confirm('Hapus artikel?');">Delete article</a>
        <a href="/Admin_seputarGigi/seputarGigi/" class="btn btn-back"><i class="fa-solid fa-arrow-rotate-left"></i> Back</a>
    </form>
</div>
<!-- end form tambah artikel -->
<script>
    const button1 = document.querySelector(".btn-toggle");
    const button2 = document.querySelector(".close-btn");
    const sidebar = document.querySelector(".sidebar");
    const konten = document.querySelector(".tambah-artikel");
    button1.onclick = () => {
